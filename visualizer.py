...
class AudioStream(object):
    ...
    def init_pyaudio(self, audio_format, time):
        ...
        self.RATE = 44100
        self.CHUNK = 1024
        self.spectrum_smoothing = 0.75
        self.wave_smoothing = 0.3
        self.time = time
    ...
    def create_pens(self):
        ...
        for r in range(0, 256):
            self.vis_pens.append(pg.mkPen(color=(r, 30, 255 - r), width=4))
    ...
    def set_plotdata(self, name, data_x, data_y, pen):
        if name in self.traces:
            ...
            if self.signalFlag and name == 'waveform':
                self.traces[name] = self.waveform.plot(pen='c', width=8)
            ...
            if self.spectrumFlag and name == 'spectrum':
                pen = pg.mkPen(color=(10, 200, 150), style=QtCore.Qt.DotLine) 
                self.spectrum.setYRange(0, 2**16, padding=0)
    ...
    def update_transform(self):
        ...
        self.fy = self.fy * self.spectrum_smoothing
        self.fy += (1 - self.spectrum_smoothing) * transformed
